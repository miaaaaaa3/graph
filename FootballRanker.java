...
public class FootballRanker {
    ...
    // to adjust output
    private static final double FORMAT_HELP = 10.0;
     ...
     * poll data. return the root mean square error of difference between actual poll results
     * and predicted results from graph.
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        int sum = 0;
        int numRanks = humanRanks.size();
        ArrayList<String> predicted = new ArrayList<>();
        for (AllPathsInfo vertex : paths) { // list and index predicted results
            predicted.add(vertex.getName());
        }
        for (int rank = 0; rank < numRanks; rank++) { // loop thru actual poll ranks
            String team = humanRanks.get(rank);
            int predRank = predicted.contains(team) ? predicted.indexOf(team) : numRanks + 1;
            sum += (rank - predRank) * (rank - predRank);
            if (showResults) { // print predicted vs actual for each team (in correct format)
        		System.out.println(String.format("%1$-" + PADDING + "s", team) + " - actual rank: "
        				+ (rank + 1) + " predicted rank: " + (predRank + 1));
            }
        }
        double result = Math.sqrt((double) sum / numRanks);
        if (showResults) {
        	System.out.println("Root Mean Square Error: " + ourFormatter.format(result));
        }
        return Math.round(result * FORMAT_HELP) / FORMAT_HELP;
    }
