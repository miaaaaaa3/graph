...
public class Graph {
    ...
    public void dijkstra(String startName) {
        dijkstraSetup(startName); // setup. includes precondition checks.
        while (!paths.isEmpty()) {
            // curr vertex has lowest cost path from start that hasn't been visited
            Vertex curr = paths.remove().dest; 
            if (curr.scratch == 0) {
                curr.scratch ++; // mark as visited
                for (Edge e : curr.adjacent) { // check each edge from curr
                    Vertex neighbor = e.dest;
                    double newCost = curr.weightedCostFromStartVertex + e.cost;
                    // if new cost is less than old cost update cost, prev, & num edges from start
                    if (newCost < neighbor.weightedCostFromStartVertex) {
                        neighbor.weightedCostFromStartVertex = newCost;
                        neighbor.prev = curr; 
                        neighbor.numEdgesFromStartVertex = curr.numEdgesFromStartVertex + 1;
                        paths.add(new Path(neighbor, newCost)); 
                    }
                }
            }
        }
    ...
    // set up conditions for dijkstra method
    private void dijkstraSetup(String startName) {
        if (startName == null) {
            throw new IllegalArgumentException("Violation of precondition. " +
                            "Vertex name may not be null.");
        }
        ...
        Vertex start = vertices.get(startName);
        // Set the cost of the start vertex to 0; all others are INFINITY
        start.weightedCostFromStartVertex = 0;
        start.numEdgesFromStartVertex = 0;
        paths.add(new Path(start, 0)); // add start to priority queue
    }
    ...
    public void findAllPaths(boolean weighted) {
        for (String name : vertices.keySet()) {
            vertices.get(name).clearPathInfo();
            if (weighted) {
                dijkstra(name);
            } else {
                findUnweightedShortestPath(name);
            }
            updatePathVars(vertices.get(name), name, weighted);
        }
        allPathsFound = true;
    }
    

    // helper method to loop through possible destination vertices & update curr's 
    // num vertices connected, sum of edges & sum of weight in shortest paths;
    // then finds longest shortest path in graph
    private void updatePathVars(Vertex curr, String name, boolean weighted) {
        for (String destName : vertices.keySet()) {
            double cost = vertices.get(destName).weightedCostFromStartVertex;
            if (cost != INFINITY && cost != 0) {
                Path currPath = getPath(destName);
                // update instance vars for vertices
                curr.numVertexConnected++; 
                curr.totalUnweightedPathLength += currPath.getNumEdgesInPath();
                if (weighted) {
                    curr.totalWeightedPathLength += currPath.weightedCost();
                } else {
                    curr.totalWeightedPathLength = curr.totalUnweightedPathLength;
                }
                if (longest == null || longest.compareTo(currPath) < 0){
                    longest = currPath; // update longest shortest path
                }
            }
        }
    }
